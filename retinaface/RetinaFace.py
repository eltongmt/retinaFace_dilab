...
def detect_faces(
    ...
    for idx, face in enumerate(det):
        ...
        yolo_width = abs(face[2] - face[0]) 
        x_center = (face[0] + (yolo_width / 2))
        yolo_height = abs(face[1] - face[3])
        y_center = (face[1] + (yolo_height / 2))

        resp[label]["facial_area"] = list((x_center / img.shape[1], y_center / img.shape[0], 
                                           yolo_width/ img.shape[1], yolo_height/ img.shape[0]))
